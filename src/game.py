...
from tiles import Monster, Player, Wall, Grass
...
class Game:
    ...
    def _load_tiles(self) -> Tuple[List[Wall], List[Grass]]:
        walls = []
        grasses = []
        ...
        for col, tiles in enumerate(world_map):
            for row, tile in enumerate(tiles):
                if tile == '1':
                    walls.append(Wall(x=row, y=col))
                elif tile == '.':
                    grasses.append(Grass(x=row, y=col))
                elif tile == 'P':
                    grasses.append(Grass(x=row, y=col))
                elif tile == 'M':
                    grasses.append(Grass(x=row, y=col))

        return walls, grasses
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_LEFT:
                    self.player.move(direction=constants.LEFT, walls=self.walls)
                elif event.key == pygame.K_RIGHT:
                    self.player.move(direction=constants.RIGHT, walls=self.walls)
                elif event.key == pygame.K_DOWN:
                    self.player.move(direction=constants.DOWN, walls=self.walls)
                elif event.key == pygame.K_UP:
                    self.player.move(direction=constants.UP, walls=self.walls)
    ...
    def restart_game(self):
        ...
        self.walls, self.grasses = self._load_tiles()
